...
const BusScreen: React.FC = () => {
  ...
  const fetchData = () => {
    fetch(
      `https://ws.infotbm.com/ws/1.0/get-realtime-pass-by-id/${line}/${stop_point}/${lineID}/${route}`
    )
      .then((response) => response.json())
      .then((data: BusData) => setBusData(data))
  ...
  const updateWaitTime = (departureTime: string): string => {
    ...
    if (["59", "60", "61", "62"].includes(lineID)) {
      moyenDeTransport = "Tram";
    }

    if (seconds < 0) return `${moyenDeTransport} à l'arrêt !`;
    if (hours > 0) return `${hours}h ${minutes}min ${seconds}s`;
    if (minutes > 0 && minutes <= 1 && seconds <= 1)
      return `${minutes} minute ${seconds} seconde`;
    if (minutes > 0 && minutes <= 1)
      return `${minutes} minute ${seconds} secondes`;
    if (minutes > 0 && seconds <= 1)
      return `${minutes} minutes ${seconds} seconde`;
  ...
  return (
    <div>
      {busData ? (
        <>
          <img
            src={`./ImagesBus/${lineID}.svg`}
            alt="logo"
            style={{ width: 50, height: 50, verticalAlign: "middle", padding: 10 }}
          />
          {busData.destinations &&
          Object.keys(busData.destinations).length !== 0 ? (
            <>
              {Object.keys(busData.destinations).map((destinationId, index) => (
                <div key={index}>
                  <h2>
                    {busData.destinations[destinationId][0].destination_name}
                  </h2>
                  {busData.destinations[destinationId].map(
                    (entry, entryIndex) => (
                      <div key={entryIndex}>
